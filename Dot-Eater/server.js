// Server File -- 12:20 pm 6/21/2017
const UP_ARROW = 0;
const RIGHT_ARROW = 1;
const DOWN_ARROW = 2;
const LEFT_ARROW = 3;
...
var allServerDots = []; // create server-side array called allServerDots.
...
io.on('connection', function (socket) {
    ...
    socket.on('newplayer', function () {
        ...
        socket.on('move', function (data) { // Update this player's recorded position in server's socket object.
        ...
        socket.on('askDot', function (data) {
            ...
            console.log(allServerDots.length);
        ...
        socket.on('disconnect', function () {
            ...
            for (i = allServerDots.length - n; i >= 0; i--) { // Go through the whole allServerDots array, where i is the index number.
                if (allServerDots[i].id == disconnectedColor) { // If the tint of allServerDots at this index number is the same as the disconnected color, ...
            ...
            // Tell all players about the dot color to delete.
            io.emit('disconnectedDots', disconnectedColor);
        });
    });
});
